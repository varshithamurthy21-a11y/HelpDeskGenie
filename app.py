...
if password_input == "admin123":
    ...
    st.sidebar.success("🔓 Administrative Access Granted")
else:
    user_role = "Employee"
    current_user_id = "emp_99"
    if password_input != "":
        st.sidebar.error("❌ Invalid Admin Password. Standard Access Enabled.")
    else:
        st.sidebar.info("👤 Standard Employee Session Active")

st.sidebar.markdown("---")

# Dynamic Menu Options: Restrict choices if user is a standard Employee
...
mode = st.sidebar.selectbox("Navigation Panel", navigation_options)

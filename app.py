...
if "audit_log" not in st.session_state:
    st.session_state.audit_log = [
        {
            "timestamp": datetime.datetime.now().strftime("%Y-%m-%d 09:14:22"),
            "action": "UNLOCK_ACCOUNT",
            "user_id": "user999",
            "status": "REJECTED",
            "details": "Missing Multi-Factor Verification."
        },
        {
            "timestamp": datetime.datetime.now().strftime("%Y-%m-%d 10:05:00"),
            "action": "CREATE_TICKET",
            "user_id": "user789",
            "status": "SUCCESS",
            "details": "Created ticket JIRA-4122"
        }
    ]
...
if mode == "Chat UI Interface":
    ...
    # Render table columns as a structured Pandas DataFrame to block parsing typos entirely
    eval_matrix = {
        "User Query": [
    ...
    for alert in st.session_state.email_alerts:
        ...
        st.markdown(f"* **Sent At:** `{alert['sent_at']}` | **Recipient:** `{alert['recipient']}` | **Severity:** **{alert['severity']}**")
        st.markdown(f"> **Incident Payload Details:** {alert['body']}")